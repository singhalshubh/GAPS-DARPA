...
#pragma clang attribute pop
    ...
    sprintf(str, "[I],[I]%d,%d\n",reqId, processed_counter);
    write(1, str, sizeof(str) );
    fprintf(f, "OUT, REQID, processed_counter : %d, %d\n", reqId, processed_counter);
    if(reqId > processed_counter){
        ...
        sprintf(str, "[TRA],[LR1]%d,%d\n", res_get_a.trailer.seq, last_processed_result);
        write(1, str, sizeof(str) );
        fprintf(f, "TRAILER, RSESULT : %d, %d\n", res_get_a.trailer.seq, last_processed_result);
        ...
        res_get_a.ret = last_processed_result;
        sprintf(str, "[RET - pc]%d,%d\n", res_get_a.ret, res_get_a.trailer.seq);
        write(1, str, sizeof(str) );
        #ifndef __ONEWAY_RPC__
        xdc_asyn_send(psocket, &res_get_a, &o_tag);
        #endif /* __ONEWAY_RPC__ */
        fprintf(f, "REQID = processed_counter\n");
    }

...
#ifndef __LEGACY_XDCOMMS__
int my_rpc_get_ewma_sync_request_counter(int* request_counter, void* psocket, void* ssocket, gaps_tag* t_tag, gaps_tag* o_tag, void * ctx, codec_map *mycmap,double x) {
...
#pragma clang attribute pop
        ...
        int status = my_xdc_recv(ssocket, &res_get_ewma, &o_tag, mycmap);
        #endif /* __ONEWAY_RPC__ */
        int respId = res_get_ewma.trailer.seq >> 2 ;
        int error = (res_get_ewma.trailer.seq >> 1)& 0x01 ;
        if(status == -1){
            tries_remaining--;
        }
        else{
...
int my_rpc_get_ewma_remote_call(int reqId, double* result, void* psocket, void* ssocket, gaps_tag* t_tag, gaps_tag* o_tag, void * ctx, codec_map *mycmap,double x) {
    int tries_remaining = 30;
    while(tries_remaining!=0){
#pragma clang attribute push (__attribute__((annotate("TAG_REQUEST_GET_EWMA"))), apply_to = any(function,type_alias,record,enum,variable,field))
        #pragma cle begin TAG_REQUEST_GET_EWMA
...
#else
int _rpc_get_ewma_sync_request_counter(int* request_counter, void* psocket, void* ssocket, gaps_tag* t_tag, gaps_tag* o_tag,double x) {
    int tries_remaining = 30;
    while(tries_remaining != 0){
#pragma clang attribute push (__attribute__((annotate("TAG_REQUEST_GET_EWMA"))), apply_to = any(function,type_alias,record,enum,variable,field))
        #pragma cle begin TAG_REQUEST_GET_EWMA
...
int _rpc_get_ewma_remote_call(int reqId, double* result, void* psocket, void* ssocket, gaps_tag* t_tag, gaps_tag* o_tag,double x) {
    int tries_remaining = 30;
    while(tries_remaining!=0){
#pragma clang attribute push (__attribute__((annotate("TAG_REQUEST_GET_EWMA"))), apply_to = any(function,type_alias,record,enum,variable,field))
        #pragma cle begin TAG_REQUEST_GET_EWMA
...
#pragma clang attribute pop
    ...
    psocket = my_xdc_pub_socket(ctx);
        ssocket = my_xdc_sub_socket_non_blocking(o_tag, ctx, 1000);
    sleep(1); /* zmq socket join delay */
        int status = my_rpc_get_ewma_sync_request_counter(&request_counter, psocket, ssocket, &t_tag, &o_tag, ctx, mycmap,x);
    if(status == 0) {
        *error = 1;
        return 0;
    }
...
#ifndef __LEGACY_XDCOMMS__
    double result;
    int status1 = my_rpc_get_ewma_remote_call(request_counter,  &result, psocket, ssocket, &t_tag, &o_tag, ctx, mycmap,x);
    if(status1 == 0){
        *error = 1;
        return 0;
    }
...
#else
    double result;
    int status1 = _rpc_get_ewma_remote_call(request_counter,  &result, psocket, ssocket, &t_tag, &o_tag,x);
    if(status1 == 0){
        *error = 1;
        return 0;
    }

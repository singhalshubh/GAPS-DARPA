...
void _handle_request_get_ewma(gaps_tag* tag) {
#ifndef __LEGACY_XDCOMMS__
    void *psocket;
    void *ssocket;
    gaps_tag t_tag;
...
#pragma clang attribute pop
...
#endif /* __LEGACY_XDCOMMS__ */
#ifndef __LEGACY_XDCOMMS__
    my_xdc_blocking_recv(ssocket, &req_get_ewma, &t_tag, mycmap);
#else
    ...
    if(reqId > processed_counter){
        ...
        last_processed_result = get_ewma(req_get_ewma.x);
        last_processed_error = error;
        res_get_ewma.trailer.seq = processed_counter << 2 | last_processed_error << 1;
        res_get_ewma.ret = last_processed_result;
        #ifndef __ONEWAY_RPC__
        xdc_asyn_send(psocket, &res_get_ewma, &o_tag, mycmap);
        #endif /* __ONEWAY_RPC__ */
    }
    else if(reqId == processed_counter){        res_get_ewma.trailer.seq = processed_counter << 2 | last_processed_error << 1;
        res_get_ewma.ret = last_processed_result;
        #ifndef __ONEWAY_RPC__
        xdc_asyn_send(psocket, &res_get_ewma, &o_tag, mycmap);
        #endif /* __ONEWAY_RPC__ */
...
int _slave_rpc_loop() {
    ...
    pthread_create(&tid[1], NULL, _wrapper_request_get_ewma, &n_tag);

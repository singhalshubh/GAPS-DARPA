...
#ifndef __LEGACY_XDCOMMS__
int my_rpc_get_a_sync_request_counter(int* request_counter, void* psocket, void* ssocket, gaps_tag* t_tag, gaps_tag* o_tag, void * ctx, codec_map *mycmap, request_get_a_datatype req_get_a, response_get_a_datatype res_get_a) {
...
int my_rpc_get_a_remote_call(int reqId, double* result, void* psocket, void* ssocket, gaps_tag* t_tag, gaps_tag* o_tag, void * ctx, codec_map *mycmap, request_get_a_datatype req_get_a, response_get_a_datatype res_get_a) {
    int tries_remaining = 5;
    while(tries_remaining!=0){
        req_get_a.dummy = 0;
...
#pragma clang attribute pop
    ...
    psocket = my_xdc_pub_socket(ctx);
        ssocket = my_xdc_sub_socket_non_blocking(o_tag, ctx, 1000);
    sleep(1); /* zmq socket join delay */
        int status = my_rpc_get_a_sync_request_counter(&request_counter, psocket, ssocket, &t_tag, &o_tag, ctx, mycmap,req_get_a, res_get_a);
    if(status == 0) {
        *error = 1;
        return 0;
    }
...
#ifndef __LEGACY_XDCOMMS__
    double result;
    int status1 = my_rpc_get_a_remote_call(request_counter,  &result, psocket, ssocket, &t_tag, &o_tag, ctx, mycmap, req_get_a, res_get_a);
    if(status1 == 0){
        *error = 1;
        return 0;
    }
